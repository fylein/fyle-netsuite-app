...
@Component({
  ...
  styleUrls: ['./memo-structure.component.scss', '../../../netsuite.component.scss']
})
export class MemoStructureComponent implements OnInit {
  ...
  getMemoStructureSettings() {
    const that = this;
    that.isLoading = true;
    that.settingsService.getGeneralSettings().subscribe(generalSettings => {
  ...
  save() {
    ...
    that.settingsService.patchGeneralSettings(that.workspaceId, memoStructure).subscribe((response) => {
      ...
      const trackingProperties = {
        ...
        oldMemoFields: that.generalSettings.memo_structure,
